...
function preload() {
  song = loadSound("../assets/audio/test.mp3");
  myFont = loadFont('../assets/FiraCode.ttf');
}


function setup() {
  let cnv = createCanvas(800, 800, WEBGL).parent('main-gallery__sketch--container');
...
function d(x, y, a, z, w, b) {
  return sqrt((x - z) * (x - z) + (y - w) * (y - w) + (a - b) * (a - b))
}
